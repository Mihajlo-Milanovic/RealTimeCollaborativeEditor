...
import { OrganizationMember, OrganizationRole } from '@/core/types/FileNode';
...
export const MemberItem: React.FC<MemberItemProps> = ({ member, onChangeRole, onRemove }) => {
    return (
        <div className="flex items-center justify-between gap-3 rounded-md border border-slate-800 bg-slate-950/70 px-3 py-2">
            ...
            <div className="flex items-center gap-2">
                <span className={`rounded border px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${roleClasses[member.role]}`}>
                    {member.role}
                </span>
                <button
